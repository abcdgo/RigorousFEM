#ifndef _DISSIPATIVEENCLOSURE_HPP_
#define _DISSIPATIVEENCLOSURE_HPP_
...
template <typename FadMapT>
typename FadMapT::VectorType enclosure(typename FadMapT::ScalarType const & currentTime, FadMapT & vField, typename FadMapT::MatrixType::RowVectorType const & x,
    ...
    for(i = 0; i < dimension; i++) {
      if(F.isDissipative(i)) { //dissipative direction we use the algorithm based on isolation from [Z3]
        ...
        if(fz[i].subset(fzn)) {
          ...
          enclosureDebug << "N(z)[i]=" << F.N(time,z)[i] << "\n";
...
typename FadMapT::VectorType enclosure( typename FadMapT::ScalarType const & currentTime, FadMapT & vField, const Tail& T, typename FadMapT::MatrixType::RowVectorType const & x,
                                        typename FadMapT::ScalarType const & step)
{
  FadMapT& nvField = vField;
  Tail T0 = nvField.getT0();
  nvField.setT0(T);
  typename FadMapT::VectorType result = capd::dynsys::enclosure(nvField, currentTime, x, step);
  nvField.setT0(T0);
  return result;
}
