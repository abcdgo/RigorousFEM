...
int main( int argc, char *argv[] )
{
    ...
    if( argc == 10 )
    {
        ...
        std::cout << "Candidate in FEM base: " << initialVector << std::endl;

        initialVector = *(fem.inverseOfP) * initialVector;//we change the base
        for( int i = 0; i < data.numberOfModes; i++ )
            initialVector[i] = initialVector[i].mid();
        std::cout << "Candidate in Diag base: " << initialVector << std::endl;

        Scalar h(-hull,hull);//needed for hull after base change
        initialVector += h;
        initTime += h;

        std::cout << "Candidate in FEM base with hull: " << initTime << std::endl;
        ...
        //std::cout << "P:" << *fem.P << std::endl;
        //std::cout << "P^-1:" << *fem.inverseOfP << std::endl;
